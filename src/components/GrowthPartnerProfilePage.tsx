      ...
      {activeTab === 'contact' ? <section aria-label="Partner profile" className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <form className="mt-6 space-y-4" onSubmit={async event => {
          ...
          <fieldset disabled={busy || emailBusy} className="min-w-0 space-y-5 disabled:opacity-60">
            ...
            <div className="sm:col-span-2 rounded-2xl border border-pink-100 bg-pink-50 p-4"><p className="text-sm font-bold text-slate-900">Social Media Share / Connections</p><p className="mt-1 text-xs text-slate-600">Save your handles, then share your Nexora partner profile.</p><button type="button" onClick={() => { const link = `${window.location.origin}/partner/profile`; void navigator.clipboard?.writeText(link); setMessage('Profile link copied.'); }} className="mt-3 rounded-xl bg-pink-600 px-4 py-2 text-xs font-bold text-white">Share Profile / Copy Link</button></div>
            </div>
