...
export function GrowthPartnerProfilePage({ client, navigate, onProfileChange }: {
  ...
  useEffect(() => {
    ...
    fetchGrowthPartnerProfile(client).then(data => {
      ...
      fetchPartnerAccountSettings(client).then(saved => {
        if (cancelled) return;
        ...
        <form className="mt-6 space-y-4" onSubmit={submitContact} noValidate>
          <fieldset disabled={busy} className="min-w-0 space-y-5 disabled:opacity-60">
            <div className="flex flex-wrap items-center gap-4 rounded-2xl bg-slate-50 p-4">
              ...
              <label className="min-w-0 basis-full text-sm font-bold text-slate-700 sm:flex-1">Profile Photo
                <input type="file" accept="image/jpeg,image/png,image/webp" className="mt-2 block w-full text-xs" onChange={async event => {
                  ...
                  // Validate BEFORE any processing: type + size, with the exact
                  // reason surfaced as a banner and a toast.
              ...
              {avatar && <button type="button" onClick={() => { setPhoto(null); setPreview(''); setRemovePhoto(true); }} className="text-xs font-bold text-slate-600 underline">Remove photo</button>}
            </div>
            ...
            <div className="grid min-w-0 gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm font-bold text-slate-600">Phone<input type="tel" maxLength={30} autoComplete="tel" placeholder="Phone number with country code" value={form.phone} onChange={e => { setForm({ ...form, phone: e.target.value }); }} aria-invalid={contactErrors.phone ? true : undefined} className={`${inputClass} ${contactErrors.phone ? errInput : ''}`} />{fieldError(contactErrors, 'phone')}</label>
              ...
              <label className="text-sm font-bold text-slate-600">WhatsApp Business Helpline<input type="tel" maxLength={30} value={business.whatsapp_phone || ''} onChange={e => setBusiness({ ...business, whatsapp_phone: e.target.value })} aria-invalid={contactErrors.whatsappPhone ? true : undefined} className={`${inputClass} ${contactErrors.whatsappPhone ? errInput : ''}`} />{fieldError(contactErrors, 'whatsappPhone')}</label>
              ...
              <label className="text-sm font-bold text-slate-600">Emergency / Alternate Phone<input type="tel" maxLength={30} value={business.alternate_phone || ''} onChange={e => setBusiness({ ...business, alternate_phone: e.target.value })} aria-invalid={contactErrors.alternatePhone ? true : undefined} className={`${inputClass} ${contactErrors.alternatePhone ? errInput : ''}`} />{fieldError(contactErrors, 'alternatePhone')}</label>
