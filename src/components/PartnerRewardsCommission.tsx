...
// ---------------------------------------------------------------------------
// The two RPC payloads below are untrusted input.
//
// The rule for this area: a missing key renders an empty state, it never
// crashes the page and it never becomes a number the database did not send.
// `money()` in particular must not turn a missing amount into a confident
// "₹0" — a partner reading ₹0 earned is materially different from one reading
// "not reported", and the amount is always PAISE (INR), never a float.
// ---------------------------------------------------------------------------
const asArray = <T,>(value: unknown): T[] => (Array.isArray(value) ? (value as T[]) : []);
const isNumber = (value: unknown): value is number =>
  typeof value === 'number' && Number.isFinite(value);
/** A count shown as data: the number, or an em dash — never a fabricated 0. */
const count = (value: unknown): string => (isNumber(value) ? String(value) : '—');
...
/** `2026-09-01` → `1/9/2026`; anything unusable renders an em dash, not
 *  "Invalid Date". */
...
/** One line per failed RPC so the cause is in the logs even though the screen
 *  keeps reviewed copy. Raw database text never reaches the user. */
const logFailure = (call: string, error: { code?: string | null; message?: string | null }) => {
  console.error(`[growth-partner] ${call} failed — ${error.code || 'database'}: ${error.message || 'no message'}`);
};
...
export function PartnerRewardsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (rpcError) { logFailure('get_my_partner_reward_dashboard', rpcError); setError('Reward progress अभी load नहीं हो सका। कृपया दोबारा कोशिश करें।'); }
  ...
  if (!data) return null;
  const milestones = asArray<RewardMilestone>(data.milestones);
  // A target the database did not send is not "1000" — it is unknown, and the
  // progress bar says so instead of drawing a number nobody computed.
  const target = isNumber(data.next_milestone) ? data.next_milestone : null;
  const progress = target !== null && isNumber(data.qualifying_shops)
    ? Math.min(100, Math.round((data.qualifying_shops / Math.max(target, 1)) * 100))
    : null;
  return <div className="space-y-5">
    <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-[#8b0a46] via-[#c20e5a] to-[#ed176f] p-6 text-white shadow-xl sm:p-8">
      ...
      <div className="mt-6"><div className="flex justify-between text-xs font-bold"><span>Next milestone: {target === null ? '—' : `${target} shops`}</span><span>{progress === null ? '—' : `${progress}%`}</span></div>
        <div className="mt-2 h-3 overflow-hidden rounded-full bg-white/20"><div className="h-full rounded-full bg-white" style={{ width: (progress ?? 0)+'%' }} /></div>
        <p className="mt-2 text-xs text-pink-100">{count(data.remaining_qualifying_shops)} qualifying shops remaining</p>
      </div>
    </section>
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      <Stat label="Verified shops" value={count(data.verified_shops)} /><Stat label="Qualifying shops" value={count(data.qualifying_shops)} />
      <Stat label="15-day cycle" value={count(data.cycles_running)} /><Stat label="Rejected shops" value={count(data.rejected_shops)} />
    </section>
    {error ? <p className="rounded-xl bg-amber-50 p-3 text-sm font-semibold text-amber-800">{error}</p> : null}
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {milestones.length === 0 ? <p className="text-sm font-semibold text-slate-500">Reward milestones अभी उपलब्ध नहीं हैं।</p> : null}
      {milestones.map((m) => <article key={m.id} className={'rounded-3xl border bg-white p-5 shadow-sm '+(m.eligible?'border-emerald-300':'border-slate-200')}>
...
export function PartnerCommissionPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (rpcError) { logFailure('get_my_partner_onboarding_rewards', rpcError); setError('Extra Onboarding Reward data अभी load नहीं हो सका।'); }
  ...
  // Same rule as above: missing keys render empty states, they never crash.
  const totals = (data.totals ?? {}) as OnboardingRewardDashboard['totals'];
  const rewards = asArray<OnboardingRewardRow>(data.rewards);
  ...
  return <div className="space-y-5">
    ...
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      <Stat label="Qualifying shops" value={count(totals.qualifying_shops)} />
      <Stat label="15-day QR collection" value={money(totals.qualifying_qr_transaction_paise)} />
      <Stat label="Company commission" value={money(totals.company_commission_paise)} />
      <Stat label="Your extra reward" value={money(totals.onboarding_reward_paise)} />
    </section>
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      {rewards.length === 0
        ...
        : <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-left text-sm">
          ...
          <tbody>{rewards.map((reward) => <tr key={reward.id} className="border-t border-slate-100">
            <td className="p-4 font-bold">{reward.shop_name || 'Shop'}</td>
            <td className="p-4 text-slate-600">{day(reward.qualification_start_date)} – {day(reward.qualification_end_date)}</td>
            ...
            <td className="p-4"><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">{String(reward.status ?? '').replaceAll('_', ' ') || '—'}</span></td>
          </tr>)}</tbody>
        </table></div>}
    </section>
  </div>;
