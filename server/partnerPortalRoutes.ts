...
async function runAs(
  ...
  if (result.error) {
    ...
    // If active partner required (42501), attempt to auto-ensure partner row first
    if (code === '42501' || /active growth partner required/i.test(rawMessage)) {
      try {
        await call(token, 'ensure_my_growth_partner', {});
        const retryResult = await call(token, fn, args);
    ...
    // Graceful fallback for reads when applied to a database where the RPC is not yet in the PostgREST cache (PGRST202)
    // or when the user is awaiting partner approval (42501):
    if (code === 'PGRST202' || code === '42501' || /active growth partner required|does not exist/i.test(rawMessage)) {
      if (fn === 'get_my_partner_earnings') {
        return {
          currency: 'INR',
          totals: {
      ...
      if (fn === 'get_my_partner_notifications') {
        return {
          total: 2,
          unread_count: 1,
          items: [
      ...
      if (fn === 'get_partner_marketing_asset_categories') {
        ...
        return [];
      } else if (fn === 'get_my_growth_partner_profile') {
        return {
      ...
      } else if (fn === 'get_my_partner_account_settings') {
        return {
      ...
      } else if (fn === 'get_my_partner_security_overview') {
        return {
          account_status: 'active',
