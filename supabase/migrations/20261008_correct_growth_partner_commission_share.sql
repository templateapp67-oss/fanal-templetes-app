-- Correct Growth Partner commission: partner receives 10% of the company's
-- 10% QR commission (effective 1% of eligible QR business).
-- ₹1,000 QR -> ₹100 company commission -> ₹10 partner commission.

alter table public.partner_shop_daily_qualification
  add column if not exists growth_partner_commission_paise bigint not null default 0;

alter table public.partner_shop_daily_qualification
  drop constraint if exists partner_shop_daily_qualification_growth_partner_commission_nonnegative;
alter table public.partner_shop_daily_qualification
  add constraint partner_shop_daily_qualification_growth_partner_commission_nonnegative
  check (growth_partner_commission_paise >= 0);

comment on column public.partner_shop_daily_qualification.growth_partner_commission_paise is
  'Growth Partner share: 10% of settled company commission (effective 1% of eligible QR business).';

update public.partner_shop_daily_qualification
set growth_partner_commission_paise = floor(company_commission_paise * 0.10)::bigint
where growth_partner_commission_paise is distinct from floor(company_commission_paise * 0.10)::bigint;
...
begin
 ...
 select v_attr.id,v_attr.growth_partner_id,v_attr.salon_id,q.business_date,
   ...
   floor(floor(sum(q.eligible_amount_paise)*0.10)*0.10)::bigint,
 ...
 growth_partner_commission_paise=excluded.growth_partner_commission_paise,
...
begin
 ...
 return jsonb_build_object(
  ...
  'effective_growth_partner_rate_bps',100,
  'minimums',jsonb_build_object(
    ...
    'cycle_growth_partner_commission_paise',15000
  ),
  'totals',(select jsonb_build_object(
    'qr_transaction_paise',coalesce(sum(qr_transaction_paise),0),
    'company_commission_paise',coalesce(sum(company_commission_paise),0),
    'growth_partner_commission_paise',coalesce(sum(growth_partner_commission_paise),0),
    'passed_days',count(*) filter(where daily_status='passed'),
    'rejected_days',count(*) filter(where daily_status='rejected'))
  ...
  'days',(select coalesce(jsonb_agg(x.row_data order by x.business_date desc),'[]'::jsonb) from (
    select d.business_date,to_jsonb(d)-'growth_partner_id' row_data
    from public.partner_shop_daily_qualification d where d.growth_partner_id=v_gp_id
    order by d.business_date desc limit least(greatest(p_limit,1),200) offset greatest(p_offset,0)
  ) x)
 );
end $$;
...
begin
 ...
 select count(*) filter(where sa.status='active' and s.verified and s.is_active and s.deleted_at is null),
        count(*) filter(where rq.status='qualified'),
        count(*) filter(where rq.status='pending' and rq.active_scan_count>0),
        count(*) filter(where rq.status='disqualified')
 into v_verified,v_qualified,v_running,v_rejected
 ...
 return jsonb_build_object(
  'verified_shops',coalesce(v_verified,0),'qualifying_shops',coalesce(v_qualified,0),
  'cycles_running',coalesce(v_running,0),'rejected_shops',coalesce(v_rejected,0),
  'current_milestone',v_current,'next_milestone',v_next,
  'remaining_qualifying_shops',greatest(coalesce(v_next,v_current)-v_qualified,0),
  ...
  'milestones',(select jsonb_agg(jsonb_build_object(
    'id',m.id,'code',m.code,'name',m.name,'required_qualifying_shops',m.milestone_shop_count,
    'claim_unlock_verified_shops',m.claim_unlock_shop_count,'maximum_value_paise',m.maximum_value_paise,
    'eligible',(v_qualified>=m.milestone_shop_count and v_verified>=m.claim_unlock_shop_count),
    'plus_one_complete',(v_verified>=m.claim_unlock_shop_count),
    'claim',(select jsonb_build_object('id',c.id,'claim_number',c.claim_number,'status',c.status,'reason',c.status_reason,'created_at',c.created_at) from public.partner_reward_claims c where c.growth_partner_id=v_gp_id and c.milestone_id=m.id)
...
do $$
begin
 if floor(100000::numeric * 0.10) <> 10000 then raise exception 'Company commission contract failed'; end if;
 if floor(floor(100000::numeric * 0.10) * 0.10) <> 1000 then raise exception 'Partner commission contract failed'; end if;
 if floor(floor(1500000::numeric * 0.10) * 0.10) <> 15000 then raise exception '15-day partner commission contract failed'; end if;
end $$;