...
begin
  ...
  select jsonb_build_object('agency_name',agency_name,'whatsapp_phone',whatsapp_phone,'city',city,'state',state,'public_bio',public_bio,'payout_method',payout_method,'payout_account_name',payout_account_name,'payout_account_number',payout_account_number,'payout_ifsc',payout_ifsc,'payout_upi_id',payout_upi_id) into r from public.partner_account_settings where partner_id=p;
  return r;
end $$;
...
begin
  if jsonb_typeof(p_patch) <> 'object' or p_patch - array['agency_name','whatsapp_phone','city','state','public_bio','payout_method','payout_account_name','payout_account_number','payout_ifsc','payout_upi_id'] <> '{}'::jsonb then raise exception 'Unsupported account setting'; end if;
  insert into public.partner_account_settings(partner_id,agency_name,whatsapp_phone,city,state,public_bio,payout_method,payout_account_name,payout_account_number,payout_ifsc,payout_upi_id)
  values(p,coalesce(p_patch->>'agency_name',''),nullif(p_patch->>'whatsapp_phone',''),coalesce(p_patch->>'city',''),coalesce(p_patch->>'state',''),coalesce(p_patch->>'public_bio',''),nullif(p_patch->>'payout_method','')::text,nullif(p_patch->>'payout_account_name',''),nullif(p_patch->>'payout_account_number',''),nullif(p_patch->>'payout_ifsc',''),nullif(p_patch->>'payout_upi_id',''))
